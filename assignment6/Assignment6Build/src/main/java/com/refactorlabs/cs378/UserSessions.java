...
public class UserSessions extends Configured implements Tool{
	...
	public static class MapClass extends Mapper<LongWritable, Text, Text, AvroValue<Session>> {
		...
		private Text logID = new Text();  
		private Session.Builder builder = Session.newBuilder();
		...
		public void map(LongWritable key, Text value, Context context)
			...
			LinkedList myList = new LinkedList();  // list to set Impression
	...
	public static class ReduceClass extends Reducer<Text, AvroValue<Session>, AvroKey<Pair<CharSequence, Session>>, NullWritable> {
		...
		public void reduce(Text key, Iterable<AvroValue<Session>> values, Context context)
			...
			for (AvroValue<Session> value : values) {
				...
				//if(copyBuild){
					outBuild = Session.newBuilder(value.datum());   //added this
					temp = Impression.newBuilder(outBuild.getImpressions().get(0)).build();
					outList.add(temp);
					//outList.add(outBuild.getImpressions().get(index));
					//outList.add(outBuild.getImpressions().remove(0));
					copyBuild = false;
				//}else{
					//outList.add(outBuild.getImpressions().get(index));
					//outList.add(outBuild.getImpressions().remove(0));
			...
			outList = sortLinkedList(outList);
		...
		public LinkedList sortLinkedList(LinkedList in){
			int index = 1;
			...
			//Impression[] tempOut = new Impression[arr.length]; 
